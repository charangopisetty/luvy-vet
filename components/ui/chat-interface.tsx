...
const gradientBaseColor = (backgroundColor: string) =>
        ...
        <AnimatePresence mode="wait">
          {shouldShowAvatar ? (
            <motion.img
              ...
              src={avatarSrc}
              alt={person.name}
              className="h-8 w-8 shrink-0 rounded-full border-[1.5px] border-white bg-zinc-800 object-cover"
              initial={{ opacity: 0, scale: 0 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0 }}
              transition={{ duration: 0.2 }}
              onError={() => setAvatarSrc('/LucyLogo.png')}
            />
          ) : (
            <div className="h-8 w-8 shrink-0" key="spacer" />
          )}
        </AnimatePresence>
        ...
        >
          {!isContinuation && (
            <motion.div
              ...
              style={{ color: chatStyle.nameColor || '#ffffff' }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
...
const ChatComponent: React.FC<ChatComponentProps> = ({ config, uiConfig = {} }) => {
  ...
  const gradientBackground = useMemo(() => {
    const base = gradientBaseColor(ui.backgroundColor);
    return `linear-gradient(to bottom, ${hexToRgba(base, 1)} 0%, ${hexToRgba(base, 0.95)} 20%, ${hexToRgba(base, 0.8)} 40%, ${hexToRgba(base, 0.4)} 70%, ${hexToRgba(base, 0)} 100%)`;
  }, [ui.backgroundColor]);

  return (
    <div
      key={key}
      className={`relative mx-auto rounded-2xl lg:mr-auto lg:ml-0 ${ui.containerClassName}`}
      style={{
    ...
    >
      {ui.backgroundColor !== 'transparent' && (
        <div
          className="pointer-events-none absolute top-0 right-0 left-0 z-10 h-32 rounded-t-2xl"
      ...
      <div
        ref={containerRef}
        className="chat-scroll-hide h-full overflow-y-scroll p-6 sm:p-8"
        style={{
          scrollbarWidth: 'none',
        ...
        <div className="flex min-h-full flex-col justify-end">
          {config.messages.map((message, index) => {
            const previousMessageComplete =
              index === 0 || completedMessages.includes(config.messages[index - 1].id);
            const previousMessage = index > 0 ? config.messages[index - 1] : null;
            const nextMessage =
              index < config.messages.length - 1 ? config.messages[index + 1] : null;
